...
//  BEGIN MOVING ALONG THE X-AXIS
	if(CNET_rand() % 2 == 0) {
	    dest.x	= CNET_rand() % (max.x - 2*CLOSE_ENOUGH)+CLOSE_ENOUGH;
	    dest.y	= now.y;
	    dest.z	= 0;
	    dx		= CNET_rand() % WALK_SPEED + 1;
	    if(dest.x < now.x)
		dx	= -dx;
	    dy		= 0;
	}
//  OR ALONG THE Y-AXIS
	    ...
	    dest.x	= now.x;
	    dest.y	= CNET_rand() % (max.y - 2*CLOSE_ENOUGH)+CLOSE_ENOUGH;
	    dest.z	= 0;
	    dy		= CNET_rand() % WALK_SPEED + 1;
	    if(dest.y < now.y)
		dy	= -dy;
	    dx		= 0;
	}
...
//  ARE WE CLOSE ENOUGH TO OUR WAYPOINT YET?
	if((rx*rx + ry*ry) <= CLOSE_ENOUGH*CLOSE_ENOUGH) {
	    paused	= true;
	    movenext	= (CNET_rand() % (MAX_PAUSE-MIN_PAUSE) + MIN_PAUSE) *
				1000000;
	}
	else {
